...
router.get('/next-important', async (req, res) => {
    try {
        const task = taskService.getNextImportantTask();
        res.json(task || null);
    }
    catch (error) {
...
router.get('/history', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const tasks = await Task_1.default.find({ userId: req.user._id }).sort({ updatedAt: -1 }).limit(10);
